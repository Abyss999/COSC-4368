...
Visualization:
  One MP4 per phase via maze_visualizer.PhaseVisualizer.
  Learning logic (Agent, MazeEnv) is NOT modified.
"""
...
def _detect_hazards(px_arr, h, w) -> dict:
    ...
    conf = _clusters(px_arr, h, w,
        lambda r,g,b: (r>210)&(g>160)&(b<70)&(np.abs(r-g)<70), min_sz=50)
...
def bfs(known: dict, start: tuple, goal: tuple,
    ...
    while queue:
        ...
        for dx, dy in ((0,-1),(1,0),(0,1),(-1,0)):
            nb = (cx+dx, cy+dy)
            if nb in came or nb not in known or nb in danger: continue
            if (cur,nb) in blocked: continue
            if passable is not None and (cur,nb) not in passable: continue
            came[nb] = cur
            queue.append(nb)
        if cur in tele:
            dest = tele[cur]
...
class Agent:
    ...
    def _nearest_frontier(self, pos: tuple):
        ...
        while queue:
            ...
            for dx,dy in ((0,-1),(1,0),(0,1),(-1,0)):
                nb = (cx+dx, cy+dy)
                if nb in visited or not (0<=nb[0]<GRID and 0<=nb[1]<GRID): continue
                if (cur,nb) in self.blocked: continue
                visited.add(nb)
                if nb not in self.known:
                    md = (abs(nb[0]-self.goal[0])+abs(nb[1]-self.goal[1])
                          if self.goal_ward and self.goal else 0)
...
# ════════════════════════════════════════════════════════
#  EPISODE RUNNER  — passes action + reward to viz.step
# ════════════════════════════════════════════════════════
def run_episodes(env: MazeEnv, agent: Agent, n: int,
                 train: bool, label: str, explore: bool = False,
    ...
    for ep in range(n):
        ...
        for turn in range(MAX_TURNS):
            ...
            # ── VIZ: pass action + reward for HUD ────────────────────────────
            if viz is not None:
                viz.step(pos, action=action, reward=reward)
...
def report_metrics(stats: list, agent: Agent, env: MazeEnv,
    ...
    if LE:
        ...
        print(f"  [Bonus] Learning Efficiency    : did not converge to 80%")
    print(bar)
    return dict(SR=SR, APL=APL, AT=AT, DR=DR, EE=EE, MC=MC, LE=LE)


# ════════════════════════════════════════════════════════
#  STATIC PATH / MAP IMAGES
# ════════════════════════════════════════════════════════
...
def save_curve(stats: list, out: Path):
    ...
    draw.text((M,M-20), "Learning Curve -- maze-alpha training (10-ep window)",
              fill=(0,0,0))
...
def _make_viz(filename: str,
    ...
    """Create a PhaseVisualizer writing to Results/<filename>.mp4"""
    if not VIZ_ENABLED:
        return None
...
def main():
    ...
    print(f"  VIZ: enabled={VIZ_ENABLED}  train={VIZ_TRAIN}  test={VIZ_TEST}")

    t0 = time.time()
    RESULTS.mkdir(exist_ok=True)

    # Training: record every 10th episode to keep file size sane over 200 eps
    viz_train  = _make_viz("training_progress.mp4",
                           step_stride=4, frames_per_episode=40,
                           episode_stride=10) if VIZ_TRAIN else None
    # Tests: all 5 episodes, show Q arrows on alpha test
    ...
    # 0 -- Pre-train on blank maze-alpha
    agent_pre = Agent()
    run_phase(MAZE_ALPHA_BLANK, agent_pre, PRETRAIN_EPISODES,
              train=True, label="MAZE-ALPHA  Pre-train (blank)", viz=None)
